$(document).ready(function () {
    ...
    var searchString = '';
    var sortKey = ''; //melyik mezore akarunk rendezni
    var sortDirection = ''; // rendezes iranya
    ...
    function fillTicketsTable(currentTickets) {
        ...
        tbody.html(''); // kiuritjuk a tbody html tag tartalmat
    ...
    function refreshTicketList() {
        ...
        urlParams.push('_limit=' + pageLimit); // push al tombhoz tudunk hozzaadni
        urlParams.push('_page=' + currentPage);
        // sima szoveges kereses lekezelese
        if (searchString.length > 0) {
            urlParams.push('q=' + searchString); // node js server query params???
        }

        // rendezes kezelese
        if (sortKey.length > 0) {
            urlParams.push('_sort=' + sortKey);  //json placeholder server kovetelmeny, kulcsszo
